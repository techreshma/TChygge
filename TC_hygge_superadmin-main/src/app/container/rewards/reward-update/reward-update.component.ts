...
export class RewardUpdateComponent implements OnInit {
  assets: any = environment.production ? 'assets/img/arrow_right_alt-24px@3x.png' : '../../../../assets/img/arrow_right_alt-24px@3x.png'
  reward_id:any;
  companyData:any = [];
  companyShow:boolean;
...
async getCompany(){
  ...
  await(this.api.getCompany().subscribe(res => {
...
async getData(){
  ...
  await(this.api.getSingleReward(formData).subscribe(res => {
    ...
    if (response.success == true){
      ...
      this.formData.company_Id         = resp.company_Id;
...
async onSelect(event) {
  ...
  if(event.addedFiles.length > 0){
    ...
    await(this.api.uploadThemeDoc(event.addedFiles[0],'reward').subscribe(res => {
      ...
      const response: any = res;
      if (response.success == true){
        this.formData.reward_Image = response.data;
