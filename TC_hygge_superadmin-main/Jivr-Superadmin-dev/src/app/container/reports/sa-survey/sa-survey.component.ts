...
@Component({
  selector: 'app-sa-survey',
  templateUrl: './sa-survey.component.html',
  styleUrls: ['./sa-survey.component.scss']
})
export class SaSurveyComponent implements OnInit {
  title = 'htmltopdf';
  ...
  displayedColumnsTitleDemo: any[] = [
    ...
    { title: 'Elligible User', chk: true, value: 'elligibleUser' },
    { title: 'End Date', chk: true, value: 'endDate' },
    { title: 'Question Count', chk: true, value: 'questionCount' },
    { title: 'Start Date', chk: true, value: 'startDate' },
    { title: 'Status', chk: true, value: 'status' },
    { title: 'Submissions', chk: true, value: 'submissions' },
    { title: 'Survey Name', chk: true, value: 'surveyName' },
    { title: 'Survey Started', chk: true, value: 'surveyStarted' },
  ];
  ...
  //Survey dashboard details
  total_surveys: any = 0;
  open_surveys: any = 0;
  completed_surveys: any = 0;
  completion_rate: any = 0;
  ...
  async getSurveyReport() {
    ...
    await this.api.getSurveyReport().subscribe(
      (res) => {
        this.ngxService.stop();
        const response: any = res;
        if (response.success == true) {
          this.total_surveys = response.data.total_survey;
          this.open_surveys = response.data.open_survey;
          this.completed_surveys = response.data.completed_survey;
          this.completion_rate = response.data.completion_rate;

          this.topFiveCompanies = JSON.stringify({
            label: response.data.topFiveCompanies.map((item) => item.companyName),
            percentage: response.data.topFiveCompanies.map((item) => item.completionRateByCompany),
            colors: ['blue'],
            height: 200,
          });

          this.data = response.data.surveyDetails;
          this.tableRecord = response.data.surveyDetails;
          this.recoverFilter = response.data.surveyDetails;
