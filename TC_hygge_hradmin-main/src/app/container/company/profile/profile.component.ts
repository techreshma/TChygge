...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //plan_Name
    this.planSelected = JSON.parse(localStorage.getItem('userData')).plan_Name.toLowerCase();
    ...
    await (this._api.showDocType().subscribe(res => {
      ...
      if (response.success) {
        ...
        response.data.forEach((el: any) => {
          el["profile"] = `${environment.apiBaseUrl}${el.doc_Image}`
        })
  ...
  async getComapny(event) {
    ...
    await (this._api.showCompanyByID(formData).subscribe(res => {
      ...
      if (response.success == true) {
        ...
        this.themeDataLicence.license_total = response.data[0].license_total;
        this.themeDataLicence.license = response.data[0].license;
  ...
  async onSelect(event) {
    ...
    if (event.addedFiles.length > 0) {
      ...
      await (this._api.uploadThemeDoc(event.addedFiles[0], 'logo').subscribe(res => {
        ...
        const response: any = res;
        if (response.success == true) {
          this.themeData.company_Logo = response.data[0];
          this.imgShow = `${environment.apiBaseUrl}${this.themeData.company_Logo}`;
  ...
  setOnOff(e, id) {
    ...
    this.workingDay[id].inTime = '';
    this.workingDay[id].outTime = '';
  }
