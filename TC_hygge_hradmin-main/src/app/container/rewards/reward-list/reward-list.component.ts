...
export class RewardListComponent implements OnInit {
  // set header column
  displayedColumns: string[] = ['reward_Image', 'reward_Name', 'reward_Points', 'acceptedReward', 'status', 'action'];
  ...
  constructor(public _access: AccessServiceService, public dialog: MatDialog, public _snackBar: MatSnackBar, public api: CommonServiceService, public ngxService: NgxUiLoaderService, public snackBar: MatSnackBar) {
  ...
  async getList() {
    ...
    await (this.api.listReward().subscribe(res => {
      ...
      if (response.success == true) {
        ...
        response.data.forEach((el: any) => {
          ...
          let expiryDate: any = moment(el.end_Date).format('YYYY-MM-DD')
          el['isExpired'] = moment(expiryDate).isAfter(today, 'month') ? '1' : '0'

          console.log(el.isExpired, el.status, el.reward_Name)
          if ((el.isExpired == 1 && el.status == 0) || (el.isExpired == 1 && el.status == 1)) {
            el.satus = 1;
          }
          else if ((el.isExpired == 0 && el.status == 1) || (el.isExpired == 0 && el.status == 0)) {
            el.status = 0
          }
        })
  ...
  async getGraphData() {
    ...
    await (this.api.graphReward({
      ...
      if (response.success == true) {
        ...
        this.departmentGraph = JSON.stringify({ label: Department, percentage: DepartmentCount, colors: this.colors, dataName: "Rewards", width: 300 })
        this.graphData = JSON.stringify({ label: ['Male', 'Female'], percentage: [graphMale, graphFemale], colors: ['#3F51B5', '#576DE6'], dataName: "Points", width: 300 })
      } else {
        // this.openErrrorSnackBar(response.message);
      }
      console.log(res);
  ...
  async getTopRedeemGraphData() {
    this.ngxService.start();
    await (this.api.graphTopRedeemReward({
      "company_Id": JSON.parse(localStorage.getItem('userData')).company_id
    }).subscribe(res => {
      this.ngxService.stop();
      const response: any = res;
      if (response.success == true) {
        ...
        this.redeemRewardGraph = JSON.stringify({ label: label, percentage: point, colors: color })
  ...
  async getRedeemGraphData() {
    this.ngxService.start();
    await (this.api.graphRedeemReward({
      "company_Id": JSON.parse(localStorage.getItem('userData')).company_id
    }).subscribe(res => {
      this.ngxService.stop();
      const response: any = res;
      if (response.success == true) {
        ...
        this.redeemLineChart = JSON.stringify({ label: label, percentage: point, colors: color })
        this.pointsLineChart = JSON.stringify({ label: label, percentage: point1, colors: color1 })
  ...
  async deleteReward(id) {
    ...
    await (this.api.deleteReward(formData).subscribe(res => {
      this.ngxService.stop();
      const response: any = res;
      if (response.success == true) {
  ...
  async updateRewardStatus(id, status) {
    ...
    await (this.api.statusReward(formData).subscribe(res => {
      this.ngxService.stop();
      const response: any = res;
      if (response.success == true) {
        this.openSnackBar(response.message);
