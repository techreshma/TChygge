...
@Component({
  selector: 'app-payslip-six',
  templateUrl: './payslip-six.component.html',
  styleUrls: ['./payslip-six.component.scss']
})
export class PayslipSixComponent implements OnInit {
  @Input() data: any;
  allData: any;
  responseData: any = [];
  demoData: any;
    ...
    await (this._api.getEmployee().subscribe(res => {
      ...
      if (response.success == true) {
        ...
        for (let item of this.responseData) {
          let total = 0
          let salAr = [];
          if (item.salaryArray != null) {
            let sal = JSON.parse(item.salaryArray)
        ...
        this.demoData = this.responseData[0];
