...
module.exports.companiesSuper_calculations = async (req, res) => {
  try {
    ...
    let totalUserOldArr = await ConnectionUtil(
      `SELECT DISTINCT(user_Id) FROM user_hrasubmit WHERE status = '1' AND company_Id = ${company_id}  ORDER BY created_At DESC `
    );
    ...
    //---------------------------------------------------mentalWellbeing over ----------------------------------------------------
     ...
     var companyDetails = await ConnectionUtil(`SELECT company_id,company_Name,company_Logo,created_At, plan_StartDate FROM company WHERE company_id='${company_id}' AND (isActive='1' AND status='1')`);
    res.status(200).json({
      success: true,
      ...
      data: {
        ...
        company_Logo: companyDetails[0].company_Logo,
        created_At: companyDetails[0].created_At,
        plan_StartDate: companyDetails[0].plan_StartDate,
        male: male,
        female: female,
