...
module.exports.Employee_Report = async (req, res) => {
  try {
    ...
    let TotalUser = await ConnectionUtil(
      `select COUNT(*) as totalUserCount from user where (isActive='1' AND status='1') ORDER BY user_id DESC`
    );
  ...
  try {
    ...
    for (let i = 0; i <= Totalcompany.length - 1; i++) {
      ...
      if (numberOfEmployees != 0) {
        var calculation =
          (count / (numberOfEmployees[0].UserCount * currentGetMonth[0])).toFixed(2) +'%';
      }
      //---- //
