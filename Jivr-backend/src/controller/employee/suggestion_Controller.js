...
module.exports.employee_SuggestionMember = async (req, res) => {
  try {
    ...
    var userDetail = await ConnectionUtil(
      `select user_id,CONCAT(first_name," ",last_name) as username  , profile_picture, mobile, email, role, company_id from user where isActive='1' AND company_id ='${companyId}'`
    );
    var userData = [];
    if (userDetail != "") {
...
module.exports.search_SuggestionMember = async (req, res) => {
  try {
    ...
    var userDetail = await ConnectionUtil(
      `select user_id,first_name , last_name ,email,mobile, profile_picture, role, company_id from user where  first_name  LIKE '%${search}%' AND  isActive='1' AND company_id ='${company_id}' `)
