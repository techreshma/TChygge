...
module.exports.User_Attendance = async (req, res) => {
  try {
    ...
    console.log(userId, companyId, location, ip_Address, time,"Request data");
    ...
    if (userDetail != "") {
      ...
      if (attendanceDetail != "") {
        ...
        let end =  time;//attendanceDetail[0].check_Out; //time
        let start = attendanceDetail[0].check_In;
        let hour_min = diff(start, end);
       
       var secs = (hour_min / 1000);
       var minute = Math.floor(secs / 60);
        ...
        let user = await ConnectionUtil(
          `update Attendance set attendance='${0}',totWork_Hours='${result}',total_time='${hour_min}',check_Out='${end}',updated_By='${userId}' ,ip_Address='${ip_Address}',checkOut_Location='${loc}'  where company_id='${companyId}' AND Attendance.user_Id = '${userId}' AND attendance='1' `
        );
        ...
        //   `select totWork_Hours,MIN(check_In) as check_In ,MAX(check_Out) as check_Out from Attendance  WHERE company_id='${companyId}'AND user_Id='${userId}' AND date='${new Date()
        //     .toISOString()
        //     .slice(0, 10)}'`
        // );
        let AttendanceTimeDetail = await ConnectionUtil(
          `select MAX(totWork_Hours) as totWork_Hours,MIN(check_In) as check_In ,MAX(check_Out) as check_Out from Attendance  WHERE company_id='${companyId}'AND user_Id='${userId}' AND date=CURDATE()`
        );
        res.status(200).json({
          success: true,
          message: "checkOut",
        ...
        var post = {
          ...
          updated_By: userId,
          // check_Out:time,
          // checkOut_Location: JSON.stringify(location)
        };
